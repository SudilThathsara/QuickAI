...
export const resumeReview = async (req, res)=>{
    try {
        ...
        if (resume.size > 5 * 1024 * 1204) {
            return res.json({success: false, message: "Resume file size exceeds allowed size (5MB)."})
        }

        const dataBufffer = fs.readFileSync(resume.path)
        const pdfData = await pdf(dataBufffer)

        const prompt = `review the ffollowing resume and provide consructive
         feedback on its strengths, weaknessses and areas for improveent. Resume
         content:\n\n${pdfData.text}`
